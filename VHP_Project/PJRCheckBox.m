...
#define kPJRCheckboxMaxFontSize 100.0
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (checkbox.checkState == PJRCheckboxStateUnchecked) {
    ...
    if (checkbox.checkState == PJRCheckboxStateUnchecked) {
        //Do Nothing
    } else if (checkbox.checkState == PJRCheckboxStateChecked) {
        [checkColor setFill];
        [[checkbox getDefaultShape] fill];
    } else if (checkbox.checkState == PJRCheckboxStateMixed) {
...
- (id)init
{
    self = [self initWithFrame:CGRectMake(0, 0, PJRCheckboxDefaultHeight, PJRCheckboxDefaultHeight)];
    return self;
}
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        _checkHeight = PJRCheckboxDefaultHeight;
        [self setup];
    }
    return self;
}
...
{
    self = [self initWithFrame:CGRectMake(0, 0, 100.0, PJRCheckboxDefaultHeight) title:title checkHeight:PJRCheckboxDefaultHeight];
    if (self) {
        CGSize labelSize;
...
{
    self = [self initWithFrame:frame title:title checkHeight:PJRCheckboxDefaultHeight];
    if (self) {
...
- (void)setup
{
    ...
    _flat = YES;
    _strokeColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:1];
    _strokeWidth = kBoxStrokeWidth * heightForCheckbox;
    _checkColor = [UIColor colorWithRed:0.0 green:0.0 blue:0 alpha:1];
    ...
    _checkAlignment = PJRCheckboxAlignmentRight;
    _checkState = PJRCheckboxStateUnchecked;
    _enabled = YES;
...
- (CGFloat)heightForCheckbox
{
    ...
    return _checkHeight == PJRCheckboxHeightAutomatic ? self.frame.size.height : _checkHeight;
}
...
- (void)autoFitFontToHeight
{
    ...
    CGFloat fontSize = kPJRCheckboxMaxFontSize;
...
- (void)layoutSubviews
{
    ...
    if (_checkAlignment == PJRCheckboxAlignmentRight) {
...
- (void)setCheckState:(PJRCheckboxState)checkState{
...
{
    if (self.type == PJRCheckboxCHECKBOX)
        self.checkState = !self.checkState;
    else
    {
        self.checkState = PJRCheckboxStateChecked;
    }
    [(FormViewController*)_parentVC performSelector:@selector(onCheckBoxSelected:) withObject:self afterDelay:0];
...
- (void)setCheckAlignment:(PJRCheckboxAlignment)checkAlignment
{
...
- (id)value
{
    if (self.checkState == PJRCheckboxStateUnchecked) {
        return uncheckedValue;
    } else if (self.checkState == PJRCheckboxStateChecked) {
        return checkedValue;
    } else {
        return mixedValue;
    }
