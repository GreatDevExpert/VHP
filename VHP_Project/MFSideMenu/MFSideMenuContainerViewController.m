...
- (void)setRightMenuViewController:(UIViewController *)rightSideMenuViewController {
    if (rightSideMenuViewController == nil) return;
...
- (void)setMenuState:(MFSideMenuState)menuState completion:(void (^)(void))completion {
    ...
    switch (menuState) {
        ...
        case MFSideMenuStateLeftMenuOpen:
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:@"sidemenu_open" object:nil];
            break;
...
- (void)rightMenuWillShow {
    if (self.rightMenuViewController == nil) return;
...
- (void) handlePan:(UIPanGestureRecognizer *)recognizer {
    ...
    if(self.panDirection == MFSideMenuPanDirectionNone) {
        ...
        else if(translatedPoint.x < 0 && self.rightMenuViewController) {
